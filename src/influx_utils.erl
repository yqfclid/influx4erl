...
-export([encode_writen_payload/3,
		 encode_writen_payload/4,
		 encode_http_path/3]).
...
%%% API
%%%===================================================================
encode_writen_payload(Measurement, Tags, Fields) ->
	encode_writen_payload(Measurement, Tags, Fields, []).

encode_writen_payload(Measurement, Tags, Fields, Options) ->
	MeasurementB = to_binary(Measurement),
	TimeB = 
		case proplists:get_value(time, Options, false) of
			true ->
				Precision = proplists:get_value(epoch, Options, ns),
...
encode_http_path(#influx_conf{protocol = http} = InfluxConf, Action, Options) ->
	#influx_conf{host = Host,
				 port = Port,
				 username = UserName,
				 password = Password,
				 database = Database} = InfluxConf,
	...
	Path1 = lists:foldl(F, <<>>, [{username, UserName}, 
								  {password, Password},
...
process_name(Module, Id) when is_atom(Module)->
    list_to_atom(atom_to_list(Module) ++ "_" ++
                 integer_to_list(erlang:phash2(Id))).

get_protocol_mod(http) ->
	{ok, influx_http};
get_protocol_mod(udp) ->
	{ok, influx_udp};
get_protocol_mod(Protocol) ->
	{error, {no_protocol_mod, Protocol}}.
%%%===================================================================
%%% Internal functions
%%%===================================================================
urlencode(Bin) ->
